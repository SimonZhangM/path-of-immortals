...
func _initialize() -> void:
	...
	_check(not state.try_start(POINT, "elsewhere", true, "arrival"), "remote arrival cannot trigger")
	_check(not state.try_start(POINT, POINT, false, "arrival"), "between-node movement cannot trigger; arrival supplies stationary eligibility")
	_check(not state.try_start("unconfigured", "unconfigured", true, "arrival"), "unconfigured signs have no invented events")
	_check(state.advance().is_empty() and record.is_empty(), "idle advance does not complete")
	_check(not state.try_start(POINT, POINT, true, "click"), "story event rejects ordinary node-click activation")
	_check(state.try_start(POINT, POINT, true, "arrival"), "story arrival starts event")
	_check(not state.try_start(POINT, POINT, true, "arrival"), "active dialogue cannot restart")
	...
	_check(not state.try_start(POINT, POINT, true, "arrival"), "completed event cannot repeat")
	var revisit := MapEventState.new(registry, record)
	_check(not revisit.try_start(POINT, POINT, true, "arrival"), "same run completion survives state replacement")
	var fresh := MapEventState.new(registry, {})
	_check(fresh.try_start(POINT, POINT, true, "arrival"), "fresh run may trigger again")
	var click_event := event.duplicate(true)
	click_event.erase("trigger")
	var click_registry := MapEventRegistry.new()
	_check(click_registry.configure([click_event], [POINT]).is_empty(), "event without trigger uses ordinary click mode")
	var click_state := MapEventState.new(click_registry, {})
	_check(not click_state.try_start(POINT, POINT, true, "arrival") and click_state.try_start(POINT, POINT, true, "click"), "ordinary event waits at the node until that node is clicked")
	var variants: Array = []
...
func _check_illustrated_event() -> void:
	...
	var record: Dictionary = {}
	var state := MapEventState.new(registry, record)
	_check(not state.try_start(second_point, second_point, true, "arrival"), "prerequisite blocks N20 before N37")
	state.try_start(POINT, POINT, true, "arrival")
	_check(state.phase == "dialogue", "legacy event starts directly in dialogue")
	while state.is_active():
		state.advance()
	_check(state.try_start(second_point, second_point, true, "arrival"), "completed N37 unlocks N20")
	...
	_check(not state.try_start(second_point, second_point, true, "arrival"), "cannot restart illustration stage")
	state.advance()
	...
	for i in 20:
		...
		if state.phase == "reward":
			_check(i == 14 and state.current_line().is_empty(), "reward interrupts after the lending line")
			state.advance()
			_check(state.phase == "reward", "ordinary advance cannot accept reward")
			state.accept_reward()
	...
	_check(not state.try_start(second_point, second_point, true, "arrival"), "completed N20 cannot repeat")
	var restored := MapEventState.new(registry, record)
	_check(not restored.try_start(second_point, second_point, true, "arrival"), "N20 stays complete across map reinstantiation")
	var fresh := MapEventState.new(registry, {})
	_check(not fresh.try_start(second_point, second_point, true, "arrival") and fresh.try_start(POINT, POINT, true, "arrival"), "new run resets completion and prerequisite")
