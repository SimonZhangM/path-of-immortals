...
func _run() -> void:
	...
	for dimensions in [Vector2i(1920, 1080), Vector2i(1280, 800), Vector2i(1280, 720), Vector2i(2560, 1080)]:
		...
		_check(is_equal_approx(panel.board_panel.global_position.x, panel.formation_panel.global_position.x) and is_equal_approx(panel.board_panel.size.x, panel.formation_panel.size.x), "array and formation share column and width")
	...
	root.size = Vector2i(1920, 1080)
	await _settle()
	var authored_items := panel.catalog.visible_entries()
	var four_items := authored_items.slice(0, 3).duplicate(true)
	var fourth: Dictionary = authored_items[0].duplicate(true)
	fourth.id = "test.fourth_card"
	fourth.acquired_at = 4
	four_items.append(fourth)
	panel.catalog.replace_entries(four_items)
	await _settle()
	var first_card: Control = panel.grid.get_child(0)
	var fourth_card: Control = panel.grid.get_child(3)
	_check(is_equal_approx(first_card.global_position.y, fourth_card.global_position.y) and fourth_card.get_global_rect().end.x <= panel.grid.get_global_rect().end.x + 1, "four cards fit completely on the same row")
	await _capture("map_inventory_four_columns")
	panel.catalog.replace_entries(authored_items)
	await _settle()
