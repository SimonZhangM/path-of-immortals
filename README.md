...
Godot 4.7.2 / GDScript / Mobile Renderer。当前 **V0.6.1：储物袋与拖拽操作修正**。
...
3. 阵盘内拖动可换位置，右键物品或拖回储物袋可收回。拖拽图像居中跟随鼠标。主角大阵盘、队友小阵盘大小固定。
...
2026-09-07：1310项逻辑、141项无窗口UI、183项真实渲染检查全部通过，导入/启动无错误。详见 [V0.6.1验证](docs/v0.6.1-validation.md)。原生虚拟鼠标在headless测试，渲染隐藏窗口不移动系统鼠标。截图和日志在忽略的artifacts/。
