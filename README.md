...
4. 按 **空格**或点击**开始战斗**后锁定背包，玄火剑每 3 游戏秒造成 10 伤害；木桩初始 100 HP，30 游戏秒被击破。
5. 战斗中再按**空格**暂停/恢复；**F1** = 0.5×，**F2** = 1×，**F3** = 2×。默认 1×，1 真实秒 = 1 游戏秒。半速每 6 真实秒攻击，正常每 3 秒攻击，2× 每 1.5 秒攻击。
...
目前通过 **484 项逻辑检查、34 项无窗口 UI 检查、39 项真实渲染场景检查**。原生鼠标拖放通过无窗口 Godot 的 `Input.parse_input_event` 路径验证，快捷键经 Viewport 输入分发验证；包含空格在布阵时开战、战斗时暂停/恢复。渲染模式检验拖放回调、键盘和画面，不操纵用户的系统鼠标。详情见 [V0.2 验证记录](docs/v0.2-validation.md)。

设计及默认窗口分辨率为 **1920×1080**，最小 1280×720，使用容器布局与 `canvas_items` 缩放。已检查 1080p、720p、16:10、超宽屏和战斗暂停画面。中文使用系统字体，尚未打包可分发字体。当前使用自制矢量占位图，没有正式美术、音效或动画。
